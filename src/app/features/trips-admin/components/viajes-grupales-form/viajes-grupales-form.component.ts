...
export class ViajesGrupalesFormComponent implements OnInit, OnDestroy {
  ...
  private uploadFilesAndCreateViaje(user: any): void {
    ...
    Promise.all(uploadPromises).then(([listImageUrl, imagenUrl, videoUrl]) => {
      if (this.isEditMode() && this.viajeId()) {
        this.updateViajeGrupal(this.viajeId()!, user, listImageUrl, imagenUrl, videoUrl);
      } else {
      this.createViajeGrupal(user, listImageUrl, imagenUrl, videoUrl);
      }
    }).catch((error) => {
      this.isLoading.set(false);
  ...
  private createViajeGrupal(user: any, listImageUrl: string | null, imagenUrl: string | null, videoUrl: string | null): void {
  ...
  private updateViajeGrupal(id: number, user: any, listImageUrl: string | null, imagenUrl: string | null, videoUrl: string | null): void {
    const formValue = this.viajeForm.value;
